...
void BDotController::get_Bdot(vector3_f& dBdt, uint16_t N){
    ...
    curr_time = t_history[hist_idx % BUFFER_LEN];
    for (uint16_t n=0; n<N; n++){
        // cast away the voltaile because we are in an atomic block
        tmp = (mag_t&) history[(hist_idx+n) % BUFFER_LEN];
        dt = t_history[((unsigned)(hist_idx+n)) % BUFFER_LEN] - curr_time; // should be negative or zero
    ...
    // get a value for old average B (Yang: will it be more efficient to store the old average B instead of recomputing here?)
    //curr_time2 = t_history[hist_idx+1];
    curr_time2 = t_history[(hist_idx+1) % BUFFER_LEN];
...
void BDotController::poll_gyro(){
    mag_t Wreading;
    int16_t tmp[3];
    imu.getRotation(&tmp[0], &tmp[1], &tmp[2]);
    Wreading.x = tmp[0];
    Wreading.y = tmp[1];
    Wreading.z = tmp[2];
    if (hist_idx ==0){
    ...
    (mag_t&) history_gyro[hist_idx] = Wreading;
}

void BDotController::get_W(vector3_f& W){
    // get a lock    
    // portMUX_TYPE MAGNETOMETER_MUTEX;
    // portENTER_CRITICAL(&MAGNETOMETER_MUTEX);
    W = history_gyro[hist_idx];
    // portEXIT_CRITICAL(&MAGNETOMETER_MUTEX);
};